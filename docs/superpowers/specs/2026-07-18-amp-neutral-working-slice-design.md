# Amp-neutral working slice — the operant seed cores run on two amps from one source — Design

- **Date:** 2026-07-18
- **Status:** REVISED during implementation (see "Revision" below); the original per-file design under it is superseded.
- **Scope:** `refrain-protocols` — the 16 generated operant cores made amp-neutral **via the generator**, plus the corpus-gate harness changes that let them pass CI.
- **Depends on:** `refrain` **v0.15.0** (released; ships `amp.reference`).
...
## Revision (2026-07-18): generator-based, all operant cores, no placement

Implementation surfaced two facts that changed the approach; the per-file design in the sections below is kept for history but superseded:

1. **`smr_theta_cz` is not hand-authored — it is one of 16 structurally-identical operant cores generated by `tools/gen_seed_protocols.py`** from a curated table, and `tests/test_generator_mode.py` regenerates the live `protocols/` dir to enforce that files match the generator. Hand-editing the file both breaks the generator's "stay consistent" invariant and gets **reverted by the generator test**. So the amp-neutral montage is a **template change**, applied to all 16 cores at once — the correct, consistent place for it. (This also explains a mid-implementation "the file keeps reverting" symptom: it was the generator test, not a rogue process.)
2. **The site does not need a `placement`.** For an operant core the site *is* the protocol's clinical identity (SMR-at-Cz vs SMR-at-C4 are separate table rows), so it stays a literal `active: "<site>"`. Only the **reference** goes amp-neutral: `reference: amp.reference`. A literal site resolves on any amp whose profile lists it; the resolver's existing `requires.channels` check then gives site-aware behaviour for free.

**The actual change:**
- **Generator** (`tools/gen_seed_protocols.py`): montage `reference: "<linked_ears>"` → `reference: amp.reference`; `requires.sample_rate` `">= 256 Hz"` → `">= 250 Hz"` (max operant band edge is 30 Hz, so 250 is ample); drop `meta.hardware = "generic"`; remove the now-unused `_SITE_REF` table. Regenerate all 16 cores.
- **Site-aware two-amp gate** (`tests/test_amp_neutral.py`): every amp-reader resolves on **q21** (a clinical amp declaring every site → `linked_ears`); on **BrainBit** it folds to `device` when the site is among BrainBit's 4 electrodes (Cz/F3/F4/Pz — 9 of 16 cores) and **fails closed** when it is not (Fz/C3/C4 — 7 of 16), which is a genuine hardware incapacity, not a defect.
- **amp=None resolvers updated**: any test that resolved these cores at `amp=None` (`test_mode_collapse.py`, `test_generator_mode.py`) now passes a clinical amp, since amp-neutral protocols fail closed without one. `build_catalog.py` is parse-only, so it is unaffected.
- **Fuzz gate**: the reader half resolves against **q21** (not BrainBit), because q21 hosts every site while BrainBit cannot.

Net: instead of one hand-massaged file, the entire operant seed set (16 cores) is amp-neutral, with the cross-device claim CI-asserted for every core whose site the amp can host.
